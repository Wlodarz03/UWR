...
def parzysta(n):
    if n%2==0: return True  # Ups, wszystkie liczby sa parzyste [!] (chyba poprawione)
...
def suma_parzystych(L):  # [!] (poprawione)
...
def histogram(L):
    ...
    for liczba in L:
        print(str(liczba)+"__" + "*" * liczba)  # [!] (poprawione)
...
def powieksz_liczby(L):
    ...
    for i in range(len(L)):
        L[i]+=1  # [!] (pass oznacza "nic nie rob") (poprawione)


def normalizuj(L):
    "funkcja odejmuje od kazdego elementu listy L srednia wartosc wszystkich elementow tej listy"
    suma = suma1(L)  # chyba jednak nie zawsze, [!] (poprawione)
    srednia = suma/len(L)
    for i in range(len(L)):
        L[i] -= srednia # Trzeba jeszcze zmodyfikowac L [!] (poprawione)
    return L


def znormalizowana(L):  # [!] (poprawione)
...
def powiekszone_parzyste_z_zerami(L):  # [!] drobny błąd (poprawione)
