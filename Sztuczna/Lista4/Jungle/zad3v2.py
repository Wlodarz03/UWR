...
class Player(object):
    ...
    def select_best_move(self, player):
        ...
        N = 20000
        simulations_per_move = N // len(moves)
        ...
        for move in moves:
            score = 0
            moves_left = simulations_per_move
            old_moves_left = N * 2
            while moves_left > 0 and old_moves_left != moves_left:
                old_moves_left = moves_left
                score_n, moves_left = self.simulate_game(move, player, moves_left)
                score += score_n
            if score > best_score:
                best_score = score
                best_move = move
    ...
    def simulate_game(self, move, player, moves_left):
        game_copy = self.copy_game(self.game)
        game_copy.do_move(move)
        current_player = 1 - player
        new_moves_left = moves_left
        while (not game_copy.victory(player) and not game_copy.victory(1 - player)) and new_moves_left > 0:
            current_move = game_copy.random_move(current_player)
            game_copy.do_move(current_move)
            current_player = 1 - current_player
            new_moves_left -= 1

        if game_copy.winner == player:
            return (1, new_moves_left)
        elif game_copy.winner == 1 - player:
            return (-1, new_moves_left)
        else:
            return (0, new_moves_left)

    def copy_game(self, game):
        new_game = Jungle()
        new_game.board = [row[:] for row in game.board]
        new_game.pieces = {0: game.pieces[0].copy(), 1: game.pieces[1].copy()}
