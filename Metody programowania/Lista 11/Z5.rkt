...
(define (fv [e : Exp]) : (Setof Symbol)
  (type-case Exp e
    ...
    [(varE x)
     (singletonS x)]
    [(letE x e1 e2)
     (unionS (fv e1)(removeS x (fv e2)))]
    [(lamE x e)
     (removeS x (fv e))]
    [(appE e1 e2)
     (unionS (fv e1) (fv e2))]))

#;(define (fv2 [e : Exp]) : (Listof Symbol)
  (type-case Exp e
    [(numE n) empty] ; liczba nie moze byc zmienna wolna
    [(opE op l r) (append (fv l) (fv r))] ; rozklad wyrazenia i rekurencyjne sprawdzenie lewego i prawego wyrazenia
    [(ifE b l r) (append (fv b) (append (fv l) (fv r)))] ; to samo konstrukcja if
    [(varE x) (list x)] ; jezeli to jest zmienna to dodajemy do listy
    [(letE x e1 e2) (append (fv e1) (removeS x (fv e2)))] ; usuwamy wszytsko to co jest zwiazane wyrazeniem let
...
(define exp2 (parse `{lambda (x) (+ x 3)}))
(define exp3 (parse `{let x 5 {if {= x 5} {let y 3 {+ z y}} {let y 2 {+ d y}}}}))
